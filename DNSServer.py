...
def Run(cmdPipe, param):
    ...
    cfg_IP_external = param['IP_external']
    ...
    dprint(__name__, 1, "HostToIntercept: {0} => {1}", intercept, cfg_IP_external)
        ...
        while True:
            ...
            try:
                ...
                if domain in intercept:
                    ...
                    paket+=str.join('',map(lambda x: chr(int(x)), cfg_IP_external.split('.'))) # 4bytes of IP
                    dprint(__name__, 1, "-> DNS response: "+cfg_IP_external)
...
if __name__ == '__main__':
    ...
    param['IP_external'] = '192.168.178.20'  # IP_external?
